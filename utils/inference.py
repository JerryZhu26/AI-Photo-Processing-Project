...
class InferenceLoop:
    ...
    def init_stage2_model(self) -> None:
        ...
        if self.args.version == "v1":
            if self.args.task == "fr":
                ...
                raise ValueError(
                    f"DiffBIR v1 doesn't support task: {self.args.task}, please use v2 by passsing '--version v2'")
        else:
    ...
    def run(self) -> None:
        ...
        from demo.ui import image_manager
        from demo.ui import out_image_manager
        # 用来创建输出路径,将args的路径创建成为self的目标路径
        self.setup()
        # We don't support batch processing since input images may have different size
        # 这里久直接获取这里面的图片,就不用地址获取图片了
        lq = image_manager.get_image()
        ...
        # 这里是进行修改过的东西
        out_image_manager.set_image(sample)
        # self.save(sample) 这里不需要save了,直接输出就行了
...
class BSRInferenceLoop(InferenceLoop):
    ...
    def init_pipeline(self) -> None:
        self.pipeline = BSRNetPipeline(self.bsrnet, self.cldm, self.diffusion, self.cond_fn, self.args.device,
                                       self.args.upscale)
...
class UnAlignedBFRInferenceLoop(InferenceLoop):
    ...
    def init_pipeline(self) -> None:
        self.pipes = {
            "bg": BSRNetPipeline(self.bsrnet, self.cldm, self.diffusion, self.cond_fn, self.args.device,
                                 self.args.upscale),
